...
* TODO Interactive Shell
  ...
  These should be exposed to the interactive shell, command line arguments, and frontend via an =exec(spacemacs|use-package-add-hook org
:pre-init
(package-initialize)
)ute("<command>", ["<arg1>", "[arg2]", "[...]"])= function. Two libraries exist, one for each of those goals. The first, =commander=, handles command line, and (spacemacs|use-package-add-hook org
:pre-init
(package-initialize)
)the next, =inquirer=, handles interactive code.
...
** Run Server
   ...
   #+END_SRC
* Complete Code
  #+BEGIN_SRC javascript :tangle server.js :nowebyes
